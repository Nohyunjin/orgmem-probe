...
Claude Code "Do you want to proceed?" 프롬프트:
...
```
Option 2 선택 시 `.claude/settings.local.json`에 **API key 포함 명령 전체 평문 저장**.

✅ 올바른 사용: shell에 env var를 먼저 export한 뒤 bare 명령 실행:
```bash
export ANTHROPIC_API_KEY=sk-ant-...
node scripts/eval-baseline.mjs --dataset ...  # 이 명령은 option 2로 whitelist 안전
```

### 자기 검토

`.claude/settings.local.json`에 `sk-`, `=sk-ant-`, `API_KEY=`, `PASSWORD=`, `TOKEN=` 등 패턴 있으면 즉시 제거 + key rotate.
